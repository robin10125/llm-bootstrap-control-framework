        ...
        for task in tasks:
            for seed in seeds:
                for arm in arms:
                    ...
                    rewrite_record = None
                    if args.initial_reward_program and bool(BIAS_ARMS[arm][0]):
                        initial_spec, active_reward_weights, initial_program_record = coach.initial_reward_program(bias_spec)
                        active_base_reward_weight = coach.base_reward_weight
                        (run_dir / "initial_bias_spec.json").write_text(json.dumps(initial_spec, indent=2) + "\n")
                        active_bias = compile_bias(initial_spec, env)
                    prior_coach = None
                    ...
                    should_rewrite = args.allow_reward_rewrite and bool(BIAS_ARMS[arm][0])
                    if should_rewrite:
                        first_cfg, second_cfg = split_cfg_for_rewrite(cfg, args.rewrite_fraction)
                        params, first_rows = train_ppo_arm(
                            env=env,
                            bias=active_bias,
                            task=task,
                            arm=arm,
                        ...
                        params, second_rows = train_ppo_arm(
                            env=env,
                            bias=active_bias,
                            task=task,
                            arm=arm,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-base-reward-weight", type=float, default=0.0)
