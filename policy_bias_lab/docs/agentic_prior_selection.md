...
`policy_bias_lab.cli.dashboard.write_dashboard` from the same payload as the checkpoint, on every evaluation. Open
...
Regenerate offline for any paused/finished run: `python -m policy_bias_lab.cli.dashboard runs/<dir>`.
...
Three arms in `policy_bias_lab/legacy/reward_modes.py`, selected by `python -m policy_bias_lab.cli.long_ppo --reward-mode ...` (no changes to the
...
- **`policy_bias_lab/cli/agentic_selection.py`** — CLI: `--out --rep freeform_staged --dof-mode --budget --n-seeds
  --patience --eps --human-analogy --llm-backend`. Emits `best_program.json` to hand to
  `python -m policy_bias_lab.cli.prior_ppo --prior-program-arm ARM=best_program.json`.
...
- **Output representation: constrained robot-derived DSL vs. free-form symbolic.** To be settled by
  a preliminary experiment (see `reports/dsl_vs_freeform.md`), not assumed. Constrained DSL = groups
