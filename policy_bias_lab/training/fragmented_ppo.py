"""Default PPO trainer using rollout fragments and staged-prior rewards.

This is the default PPO path for prior-program training:
...
* Learned policy outputs can gate how much of the action prior is applied.
...
# Compatibility name for callers that treat policy_bias_lab.training.fragmented_ppo as the default PPO module.
PPOBiasConfig = FragmentedStagePPOConfig
...
def train_fragmented_stage_ppo(
    *,
    ...
    if cfg.warmup_compile:
        ...
        stage0 = jp.zeros((cfg.envs,), dtype=jp.int32)
        _state, _stage, traj, last_value, _summary, _eval_traj = collect(
            params, state0, stage0, jax.random.PRNGKey(seed + 100), base_reward_weight,
            action_prior_weights)
    ...
    prior_stage_idx = jp.zeros((cfg.envs,), dtype=jp.int32)
    ...
    for it in range(int(cfg.iters)):
        ...
        state, prior_stage_idx, traj, last_value, eval_summary, _eval_traj = collect(
            params, state, prior_stage_idx, ck, base_reward_weight, action_prior_weights)
        data = _ppo_data_from_traj(traj, last_value, cfg.gamma, cfg.lam)
...
def evaluate_fragmented_policy(
    *,
    ...
    prior_stage_idx = jp.zeros((eval_cfg.envs,), dtype=jp.int32)
    ...
    for i in range(int(env.horizon) // int(eval_cfg.fragment_steps)):
        state, prior_stage_idx, traj, _last_value, eval_summary, _eval_traj = collect(
            params, state, prior_stage_idx, jax.random.PRNGKey(seed + 1 + i), base_reward_weight,
            action_prior_weights)
    ...
    if return_obs:
        out["eval_obs"] = np.concatenate([np.asarray(t[0]) for t in traj_parts], axis=0)
...
def make_fragment_collect(
    *,
    ...
    def policy_step(params, obs, prior_stage_idx, key, action_prior_weights):
        ...
        if cfg.use_action_prior and bias.prior_fn is not None:
            if bias.prior_step_fn is not None:
                prior, next_prior_stage_idx = jax.vmap(
                    lambda o, c: bias.prior_step_fn(o, action_prior_weights, c)
                )(obs, prior_stage_idx)
            else:
                prior = jax.vmap(lambda o: bias.weighted_action_prior(o, action_prior_weights, task))(obs)
                next_prior_stage_idx = prior_stage_idx
            env_action = jp.clip(residual + prior_scale * prior, -1.0, 1.0)
        else:
            env_action = jp.clip(residual, -1.0, 1.0)
            next_prior_stage_idx = prior_stage_idx
        return policy_action, logp, value, env_action, prior_scale, next_prior_stage_idx

    def collect(params, state, prior_stage_idx, key, base_reward_weight, action_prior_weights):
        def body(carry, _):
            state, prior_stage_idx, key = carry
            key, ak = jax.random.split(key)
            policy_action, logp, value, env_action, prior_scale, next_prior_stage_idx = policy_step(
                params, state.obs, prior_stage_idx, ak, action_prior_weights)
            ...
            return (nstate, next_prior_stage_idx, key), (
                state.obs,
                policy_action,
                logp,
                value,
        ...
        (state, prior_stage_idx, key), traj = jax.lax.scan(
            body, (state, prior_stage_idx, key), None, length=int(cfg.fragment_steps))
        _mean, _log_std, last_value = net.apply(params, state.obs)
        eval_traj = traj[10]
        eval_summary = _eval_summary(eval_traj)
        return state, prior_stage_idx, traj, last_value, eval_summary, eval_traj
