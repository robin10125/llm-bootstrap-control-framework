...
#
# NOT started automatically -- launch with:
#   cd /home/robin/Documents/agent-mini-script-control/llm-framework
#   nohup bash policy_bias_lab/experimental/prior-generation-study/run_critic_vs_baseline_3seed.sh \
#     > runs/critic_vs_baseline_3seed/driver.log 2>&1 &
set -u
cd /home/robin/Documents/agent-mini-script-control/llm-framework
...
for S in $SEEDS; do
  ...
  if [ ! -f "$OUT/$B/final_report.json" ]; then
    $PY -m policy_bias_lab.run_long_ppo --out $OUT/$B --program $PROG --task lift \
